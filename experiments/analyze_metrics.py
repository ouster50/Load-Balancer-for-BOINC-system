...
def main():
    ...
    completed = [
        ...
        if get_as_float(row, "received_time") > 0
        and int(get_as_float(row, "outcome")) == 1
        and int(get_as_float(row, "exit_status")) == 0
    ]
    response = [get_as_float(row, "received_time") - get_as_float(row, "wu_create_time") for row in completed]
    service = [get_as_float(row, "received_time") - get_as_float(row, "sent_time") for row in completed]
    queueing = [get_as_float(row, "sent_time") - get_as_float(row, "wu_create_time") for row in completed]
    ...
    for host, rows in sorted(by_host.items()):
        cpu_time = sum(get_as_float(row, "cpu_time") for row in rows)
        elapsed_time = sum(get_as_float(row, "elapsed_time") for row in rows)
        ncpus = max(1.0, get_as_float(rows[0], "p_ncpus"))
        ...
        node_rows.append({
            ...
            "p_ncpus": ncpus,
            "p_fpops": get_as_float(rows[0], "p_fpops"),
            "completed": len(rows),
    ...
    received_times = [get_as_float(row, "received_time") for row in completed]
    arrival_times = [get_as_float(row, "wu_create_time") for row in completed]
    deadline_misses = sum(get_as_float(row, "received_time") > get_as_float(row, "report_deadline") for row in completed)
    by_class = {}
    for work_class in sorted({row.get("work_class", "unknown") for row in completed}):
        ...
        by_class[work_class] = {
            ...
            "response_time_seconds": distribution(get_as_float(row, "received_time") - get_as_float(row, "wu_create_time") for row in class_rows),
            "service_time_seconds": distribution(get_as_float(row, "received_time") - get_as_float(row, "sent_time") for row in class_rows),
        }
    estimate_relative_errors = []
    for row in completed:
        elapsed = get_as_float(row, "elapsed_time")
        flops = get_as_float(row, "flops_estimate")
        fpops_est = get_as_float(row, "rsc_fpops_est")
    ...
    metrics = {
        ...
        "makespan_seconds": (max(received_times) - min(arrival_times) if received_times and arrival_times else None),
        "deadline_misses": deadline_misses,
        "deadline_miss_rate": (deadline_misses / len(completed) if completed else None),
        "load_utilization_cv": covariation(utilizations),
        "jain_fairness_completed_tasks": jain_index(completion_counts),
        "jain_fairness_by_work_class": {
            work_class: jain_index([sum(row.get("work_class") == work_class for row in rows) for rows in by_host.values()])
            for work_class in ("short", "medium", "long")
        },
        "runtime_estimate_relative_error": distribution(estimate_relative_errors),
        "cpu_efficiency": (sum(get_as_float(row, "cpu_time") for row in completed) / max(0.001, sum(get_as_float(row, "elapsed_time") for row in completed))),
        "scheduler_decision_time_ms": distribution(decision_ms),
        "scheduler_overhead_percent": (sum(decision_ms) / 1000.0 / observation * 100.0 if decision_ms else 0.0),
